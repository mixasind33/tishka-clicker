...
// AUDIO
let soundOn = true; // Legacy support, now using state.settings
let musicPlayer = new Audio('https://cdn.pixabay.com/download/audio/2022/11/22/audio_febc508520.mp3'); // Lo-fi royalty free
...
function getVolume(type) {
    let master = state.settings.master / 100;
    let vol = state.settings[type] / 100;
    return master * vol;
}

// Optimization: Sound Pool
...
function playSound(type) {
    ...
    if (state.settings.sfx > 0) {
        try {
            let vol = getVolume('sfx');
            if (type === 'click') {
                // Optimized pool playback
...
function updateMusicVolume() {
    musicPlayer.volume = getVolume('music') * 0.5; // Max 0.5 to not overpower
...
function updateUI() {
    ...
    let currentLeague = "Bronze"; for (let l of LEAGUES) if (state.coins >= l.min) currentLeague = l.name;
    document.getElementById('league-disp').innerText = currentLeague + " League";
    let stars = ""; for (let i = 0; i < state.prestige; i++) stars += "⭐️";
    document.getElementById('prestige-disp').innerText = stars;
    ...
    if (document.getElementById('factory')) {
        ...
        // Sell button
        let milkVal = Math.floor(state.factory.milk * 10); // 10 coins per liter
...
function checkOffline() {
    ...
    if (state.factory.cows > 0 && factorySeconds > 60) {
        // Calculate how much milk could be produced
        ...
        if (actualProduced > 0) {
            ...
            // Note: We don't auto-sell milk, it just fills up the truck
        }
    }
...
circle.addEventListener('pointerdown', (e) => {
    ...
    let power = state.clickPower * state.multiplier * activeBuffs.click;
    let isCrit = Math.random() < 0.05;
    if (isCrit) power *= 5;
    state.coins += power; state.energy--; state.stats.totalClicks++; state.quests.clicks++;
    spawnText(e.clientX, e.clientY, format(power), isCrit ? 'var(--danger)' : 'white', isCrit);
    playSound(isCrit ? 'crit' : 'click');
    if (isCrit) { circle.classList.add('shake'); setTimeout(() => circle.classList.remove('shake'), 300); }
    updateUI(); updateQuests();
});
...
setInterval(() => {
    ...
    if (state.energy < state.maxEnergy) { state.energy += 5; if (state.energy > state.maxEnergy) state.energy = state.maxEnergy; }
    updateUI();
}, 1000);
...
// BOSS
// BOSS
const BOSS_HP_BASE = 20000;
const BOSS_TIME = 15.00;

window.startBossCapture = function () {
    // Transition effect
    let overlay = document.getElementById('transition-overlay');
    overlay.style.opacity = 1;
    playSound('meow'); // Sound effect
...
function startBoss() {
    ...
    let timeLeft = BOSS_TIME;
    document.getElementById('boss-timer').innerText = timeLeft.toFixed(2);

    if (bossTimerInt) clearInterval(bossTimerInt);

    bossTimerInt = setInterval(() => {
        timeLeft -= 0.1;
        document.getElementById('boss-timer').innerText = Math.max(0, timeLeft).toFixed(2);
...
window.hitBoss = function (e) {
    ...
    // Critical hit chance on boss too? Why not.
    let dmg = state.boss.damage;
    let isCrit = false;
    if (Math.random() < 0.1) { dmg *= 2; playSound('crit'); isCrit = true; } // 10% crit chance for x2
    else playSound('click');

    bossHP -= dmg;

    // VISUAL FEEDBACK (New)
    // Get boss position center for text spawn
    let bossRect = document.querySelector('.modal .buy-btn').getBoundingClientRect();
    // Just spawn around center screen but slightly randomized
...
window.upgradeBossDamage = function () {
    ...
    if (state.coins >= cost) {
        ...
        state.boss.level++; // Tracking upgrades count essentially
        playSound('success');
        updateUI();
        save();
    }
...
function endBoss(isWin) {
    ...
    if (isWin) {
        let reward = 50000 * (state.prestige + 1); // Bigger reward for harder boss
        state.coins += reward;
...
function renderLeaderboard() {
    ...
    let fakeBots = [{ name: 'CatMaster', score: 50000000 }, { name: 'Murzik2000', score: 12000000 }, { name: 'Вы', score: state.coins, me: true }, { name: 'NoobCat', score: 150 }];
    fakeBots.sort((a, b) => b.score - a.score);
    fakeBots.forEach((bot, index) => { list.innerHTML += `<div class="leader-row ${bot.me ? 'me' : ''}"><div class="leader-rank">#${index + 1}</div><div class="leader-name">${bot.name}</div><div class="leader-score">${format(bot.score)}</div></div>`; });
}

function updateQuests() {
    let today = new Date().getDay(); if (state.quests.lastDay !== today) state.quests = { lastDay: today, clicks: 0, opened: 0, claimed: [] };
...
function switchTab(id, btn) {
    ...
    // Hide energy bar on non-game tabs
...
function openModal(id) { document.getElementById(id).classList.add('open'); if (id === 'modal-settings') document.getElementById('stats-disp').innerText = `Кликов: ${state.stats.totalClicks} | Боссов: ${state.stats.bossesKilled}`; }
function closeModal(id) { document.getElementById(id).classList.remove('open'); }
// function toggleSound() { soundOn = !soundOn; document.getElementById('sound-state').innerText = soundOn ? 'ВКЛ' : 'ВЫКЛ'; }
...
function checkDailyReward() {
    ...
    // Logic: 
    // If diff > 24h AND diff < 48h -> Next streak day
    // If diff > 48h -> Reset streak to 0
    // If diff < 24h -> Already claimed today (wait)

    let hours = diff / (1000 * 60 * 60);

    if (hours > 48) { state.daily.streak = 0; } // Missed a day

    // If it's been more than 20 hours (allowing some leeway) since last claim, show modal
    if (hours > 20) {
...
function renderDailyGrid() {
    ...
    // Cap visual streak at 6 (index 0-6)
    if (currentStreak > 6) currentStreak = 0; // Should reset after day 7 claim actually
    ...
    // Logic check again to ensure safety
...
window.claimDailyReward = function () {
    ...
    state.coins += reward;
    state.daily.streak++;
    if (state.daily.streak >= 7) state.daily.streak = 0; // Loop back
...
function spawnLaserDot() {
    if (document.querySelector('.laser-dot')) return; // Только одна точка за раз
    let dot = document.createElement('div'); dot.className = 'laser-dot';

    // Random position limits (avoiding full edge to prevent overflow)
...
// Перехватываем открытие модалки, чтобы инициализировать игру
...
function initClawGame() {
    ...
    prizes.forEach((p, i) => {
        ...
        // Track item bounds logic (simplified)
        clawItems.push({
            el: el,
            type: p.type,
            val: p.val,
            center: (i * 18) + 10 // approximate center %
        });
    });
...
window.dropClaw = function () {
    ...
    setTimeout(() => {
        // Check collision (logic: convert px left to % and check against items)
        ...
        let caughtItem = null;
        // Simple collision: if arm is close to item center
        ...
        setTimeout(() => {
            if (caughtItem) {
                if (caughtItem.type === 'coin') {
                    let win = caughtItem.val;
                    state.coins += win;
                    showRewardPopup(win, "Кран-машина");
...
setInterval(() => {
    if (Math.random() < 0.4 && document.visibilityState === 'visible') spawnLaserDot();
}, 5000);